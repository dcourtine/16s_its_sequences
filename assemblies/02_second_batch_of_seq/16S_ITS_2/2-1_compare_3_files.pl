...
foreach my $k(sort {$a cmp $b} keys(%A71R)){
      if(defined($A4F{$k})){              ##vérifier que le nom de la séquence est présente dans les 4 fichiers
#            print"$k\n";
            if (defined($A1492R{$k})){
#                  if(defined($A958R{$k})){
                        #print OUT "$k\n";
...
open(IN1, "MultiFasta-A1492R.fasta") or die "erreur $!";
my %h1492;
while(<IN1>){
      chomp;
      if (substr ($_, 0,1) eq ">") {
...
#Writing output:
open (OUT1, ">MultiFasta-A1492R.fasta.final") or die "erreur $!";
...
close OUT1;
             ...
             ##############################           
open(IN2, "MultiFasta-A71R.fasta") or die "erreur $!";
my %h71;
$seq="";
$nom="";
while(<IN2>){
...
#Writing output:
open (OUT2, ">MultiFasta-A71R.fasta.final") or die "erreur $!";
for (my $i=0;$i<=$#liste_nom_seq;$i++){   #Parcours du tableau contenant tous les noms de seq
...
close OUT2;
            #################################
open(IN3, "MultiFasta-A4F.fasta") or die "erreur $!";
my %h8;
$seq="";
$nom="";
while(<IN3>){
...
#Writing output:
open (OUT3, ">MultiFasta-A4F.fasta.final") or die "erreur $!";
for (my $i=0;$i<=$#liste_nom_seq;$i++){   #Parcours du tableau contenant tous les noms de seq communes
      ...
      foreach my $k(sort {$a cmp $b} keys(%h8)){        #parcours du hash h8 pour avoir les seq des seq communes   
           ...
           if ($liste_nom_seq[$i] =~m/$k/ ){          
                my $new_name=$k." A4F";               #avant: $new_name=">".$k." A8F";
                $new_name =~s/_ / /;
				print OUT3 "$new_name\n$h8{$k}\n";
           }
      }
